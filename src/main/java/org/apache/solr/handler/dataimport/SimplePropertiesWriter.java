...
import org.apache.solr.handler.dataimport.legacy.SolrPathsV8;
...
//                configDir = SolrResourceLoader.locateSolrHome().toString();
                configDir = SolrPathsV8.locateSolrHome().toString();
            } else {
    ...
    private File getPersistFile() {
        if (new File(filename).isAbsolute() || configDir == null) return new File(filename);
